...
def h_text(m):
    if "кто создатель" in m.text.lower() or "кто твой папа" in m.text.lower():
    ...
    try:
        if call.data=="btn_help": h_help(call.message)
            ...
            if ADMIN_ID and str(ADMIN_ID).isdigit():
                bot.send_message(int(ADMIN_ID), f"🚨 РЕПОРТ от @{call.from_user.username}")
