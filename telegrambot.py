...
def get_env_smart(*names):
    ...
    for key, value in os.environ.items():
        if key.startswith('BOT') and len(value) > 30:
            if ':' in value:
                return value.strip()
        if key.startswith('GRO') and len(value) > 20:
            if value.startswith('gsk_'):
                return value.strip()
    return None

TELEGRAM_TOKEN = get_env_smart(
    'TELEGRAM_TOKEN','BOT_TOKEN','BOT','TG_TOKEN')
GROQ_API_KEY = get_env_smart(
    'GROQ_API_KEY','GROQ','GROQ_KEY')
if not TELEGRAM_TOKEN:
    TELEGRAM_TOKEN = 'ВСТАВЬ_ТОКЕН'
if not GROQ_API_KEY:
    GROQ_API_KEY = 'ВСТАВЬ_GROQ'
...
SYSTEM_PROMPT = "Ты — Даун213, теплый позитивный бот. Создатель Максим @MakSon4ikk_228. Йоу редко."
...
user_cooldown = {}
chat_stats = {
    'total_requests':0,'photos':0,
    'texts':0,'errors':0,'images':0,
...
def get_chat_memory(chat_id):
    if chat_id not in chat_memories:
        ...
        if len(chat_memories) > MAX_CHATS:
            oldest = min(
                chat_last_active, key=chat_last_active.get)
...
def add_memory(chat_id, role, text):
    ...
    mem.append({
        'role': role,
        'content': text[:MAX_TEXT_LEN].replace(chr(10),' ').strip()
    })
...
def get_stats_text():
    ...
    return f'Чатов: {total} | Сообщ: {msgs} | Запросов: {chat_stats["total_requests"]} | Картинок: {chat_stats["images"]}'
...
def is_spam(uid):
    now = time.time()
    if now - user_cooldown.get(uid,0) < 1.3:
        return True
...
def enhance_prompt_for_image(user_prompt):
    ...
    if groq_client:
        if len(user_prompt) < 70 or 'а' in low:
            try:
                comp = groq_client.chat.completions.create(
                    model=TEXT_MODEL_FALLBACK,
                    messages=[
                        {"role": "system",
                         "content": "Translate to English image prompt. If rtx, ртх, видеокарта, 5090 -> Nvidia RTX 5090 graphics card, photorealistic, triple fan, black, studio lighting, white background, 8k. Answer ONLY prompt."},
                        {"role": "user",
                         "content": user_prompt}
                    ],
                    temperature=0.2,
                ...
                pass
    if 'ртх' in low or 'rtx' in low or 'видюха' in low:
        return f"Nvidia RTX 5090 graphics card, photorealistic, {user_prompt}, 8k"
    if 'видеокарта' in low or '5090' in low:
        return f"Nvidia RTX 5090 graphics card, triple fan, black, studio, 8k, {user_prompt}"
...
def generate_image_pollinations(prompt):
    ...
    try:
        ...
        urls = [
            f"https://image.pollinations.ai/prompt/{safe_prompt}?width=1024&height=1024&nologo=true&seed={random.randint(1,999999)}&model=flux",
...
async def ask_groq(chat_id, text, image_b64=None):
    ...
    if image_b64:
        ...
        messages.append({
            'role':'user',
            'content':[
                {'type':'text','text':text or 'Что на фото?'},
                {'type':'image_url','image_url':{'url':f'data:image/jpeg;base64,{image_b64}'}}
            ]
        })
        model_list = [
            VISION_MODEL, VISION_MODEL_2,
            FALLBACK_VISION_MODEL,
            FALLBACK_VISION_MODEL_2,
            FALLBACK_VISION_MODEL_3,
            FALLBACK_VISION_MODEL_4,
            TEXT_MODEL
        ]
    else:
...
async def start_h(update, context):
    ...
    await update.message.reply_text(
        f"Привет, {user}! Жми 🎨 Картинка и пиши что рисовать!",
        reply_markup=MAIN_KB)

async def help_h(update, context):
    await update.message.reply_text(
        "🎨 Картинка -> пишешь 'кота' или 'ртх 5090' и он рисует.",
        reply_markup=MAIN_KB)
...
async def clear_h(update, context):
    ...
    await update.message.reply_text(
        "Память очищена!", reply_markup=MAIN_KB)
...
async def image_h(update, context):
    ...
    if not prompt:
        ...
        await update.message.reply_text(
            "Что нарисовать? Напиши например: кота, rtx 5090 🎨",
            reply_markup=MAIN_KB)
        return
    await context.bot.send_chat_action(
        update.effective_chat.id,'upload_photo')
    await update.message.reply_text(
        f"Рисую: {prompt}... ⏳", reply_markup=MAIN_KB)
    img = generate_image_pollinations(prompt)
    if img:
        chat_stats['images']+=1
        await update.message.reply_photo(
            photo=img, caption=f"Готово! {prompt}",
            reply_markup=MAIN_KB)
    else:
        await update.message.reply_text(
            "Не удалось, попробуй еще раз.", reply_markup=MAIN_KB)

async def limit_h(update, context):
    args=context.args
    if not args:
        await update.message.reply_text(
            "Используй /limit пароль", reply_markup=MAIN_KB)
        return
    if args[0]!=ADMIN_PASSWORD:
        await update.message.reply_text(
            '❌ Неверный пароль', reply_markup=MAIN_KB)
        return
    await update.message.reply_text(
        f"Всего: {chat_stats['total_requests']}\n{get_stats_text()}\n{format_time_full()}",
        reply_markup=MAIN_KB)
...
async def text_h(update, context):
    ...
    if low in ['🎨 картинка', 'картинка']:
        context.user_data['awaiting_image'] = True
        await update.message.reply_text(
            "Что нарисовать? Напиши: кота, rtx 5090, машину 🎨",
            reply_markup=MAIN_KB)
        return
    if 'картинка' in low and len(low) < 15:
        context.user_data['awaiting_image'] = True
        await update.message.reply_text(
            "Что нарисовать? Напиши: кота, rtx 5090 🎨",
            reply_markup=MAIN_KB)
        return
    awaiting = context.user_data.get('awaiting_image', False)
    is_image_request = awaiting or any(
        x in low for x in [
            'нарисуй','сгенерируй картинку',
            'сделай картинку','нарисовать','сгенерируй'])
    if is_image_request:
        prompt = txt
        if awaiting:
            prompt = txt
            ...
            for word in [
                ...
                if word in low:
                    parts = re.split(
                        word, txt, flags=re.IGNORECASE)
                    if len(parts) > 1 and parts[1].strip():
                        prompt = parts[1].strip(' :,-')
                    else:
                        context.user_data['awaiting_image'] = True
                        await update.message.reply_text(
                            "Что именно нарисовать? Напиши: кота",
                            reply_markup=MAIN_KB)
                        return
                    break
        if not prompt or len(prompt) < 2:
            context.user_data['awaiting_image'] = True
            await update.message.reply_text(
                "Что именно нарисовать? Пример: кота, rtx 5090",
                reply_markup=MAIN_KB)
            return
        await context.bot.send_chat_action(
            update.effective_chat.id,'upload_photo')
        ...
        if img:
            ...
            await update.message.reply_text(
                "Не вышло, попробуй еще раз.", reply_markup=MAIN_KB)
        return
    ...
    if 'создатель' in low or 'кто тебя сделал' in low:
        kb_inline = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "👑 Профиль создателя",
                url="https://t.me/MakSon4ikk_228")]])
        await update.message.reply_text(
            "Меня создал Максим @MakSon4ikk_228!",
            reply_markup=MAIN_KB)
        await update.message.reply_text(
            "Вот его профиль 👇", reply_markup=kb_inline)
        return
    if 'забыть' in low:
        clear_memory(update.effective_chat.id)
        context.user_data['awaiting_image']=False
    ...
    if 'админу' in low:
        await update.message.reply_text(
            "Напиши @MakSon4ikk_228", reply_markup=MAIN_KB)
        return
    if 'помощ' in low:
...
async def photo_h(update, context):
    if is_spam(update.effective_user.id):
        await update.message.reply_text(
            'Подожди с фотками ⏳', reply_markup=MAIN_KB)
        return
    ...
    try:
        ...
        await update.message.reply_text(
            'Ошибка с фото', reply_markup=MAIN_KB)
