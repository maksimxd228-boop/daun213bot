...
def gen_img_sync(prompt):
    ...
    is_cat = 'кот' in low or 'кош' in low
    ...
    elif is_cat and ('латяо' in low or 'латьяо' in low or 'ест' in low or 'кушает' in low):
        final = "cute fluffy orange cat eating food from bowl, photorealistic, highly detailed, 8k, adorable cat"
    elif is_cat:
        final = "cute fluffy cat, photorealistic, highly detailed, 8k, adorable"
    else:
    ...
    try:
        ...
        pass
    final = final[:400]
    logger.info(f"GEN_IMG final={final}")
    try:
        import urllib.parse, random, urllib.request, time
        from io import BytesIO
        ...
        urls=[
            ...
            f"https://image.pollinations.ai/prompt/{safe}?width=768&height=768&nologo=true&seed={seed+2}&model=flux-realism&enhance=false",
            f"https://image.pollinations.ai/prompt/{safe}?width=768&height=768&nologo=true&seed={seed+3}&model=turbo&enhance=false&nofeed=true",
        ]
        for url in urls:
            for attempt in range(2):
                try:
                    ...
                    with urllib.request.urlopen(req,timeout=35) as r:
                        d=r.read()
                        if len(d) < 8000:
                            logger.warning(f"Too small {len(d)}")
                            continue
                        try:
                            bio=BytesIO(d)
                            img=Image.open(bio)
                            ...
                            logger.warning(f"PIL fail {e}")
                            continue
                except Exception as e:
                    logger.warning(f"IMG fetch fail {e}")
                    time.sleep(0.7)
                    continue
    except Exception as e:
...
async def ask(cid,text,b64img=None):
    if client is None:
        ...
        return 'Мозг не подключен. Проверь GROQ_API_KEY в Render.'
    stats['total']+=1
    mem=get_mem(cid)
    utc,riga,msk=get_time()
